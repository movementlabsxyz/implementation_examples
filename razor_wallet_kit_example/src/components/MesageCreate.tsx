...
const MessageCreate: React.FC<MessageDisplayProps> = ({ setMessage }) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await signAndSubmitTransaction({ payload: { function: SET_MESSAGE_FUNCTION, functionArguments: [inputMessage] } })
  ...
  return (
    <div className="message-create-container">
      <form onSubmit={handleSubmit} className="message-form">
        <label htmlFor="messageInput" className="message-label">New Message:</label>
        <input
          type="text"
          id="messageInput"
          value={inputMessage}
